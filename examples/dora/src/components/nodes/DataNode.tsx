...
export function ComputeNode(props) {
  return (
    <NodeContainer>
      <NodeHeader />
      <NodeBody>
        <NodeInputs />
        <Focusable className="mx-3">{props.children}</Focusable>
        <NodeOutputs />
      </NodeBody>
    </NodeContainer>
  );
...
export function NodeHeader({ className = "", ...props }) {
  ...
  return (
    <header className={`py-3 px-4 flex flex-col items-center ${className}`} {...props}>
      {meta.category && (
        <div className={"text-xs text-gray-500 font-normal uppercase"}>{meta.category}</div>
      )}
      {meta.title && <div className={`text-lg text-gray-600 font-semibold`}>{meta.title}</div>}
    </header>
  );
}
...
export function NodeInputs() {
  ...
  return (
    <div className="flex flex-col gap-2">
      {inputs.length ? inputs.map(id => <NodeInput inputID={id} key={id} />) : <div></div>}
    </div>
  );
}
...
export const getPinIsAcceptingConnections = atomFamily((id: string) => get => {
  ...
  if (fromPin === null) {
    ...
    if (fromPin === id) {
      ...
      get(model.getPinMetadata(fromPin)).parentNode === get(model.getPinMetadata(id)).parentNode ||
      get(model.getPinMetadata(fromPin)).type === get(model.getPinMetadata(id)).type
    ) {
      return false;
    }
    return true;
...
export function NodeInput({ inputID }) {
  ...
  const isActive = hasConnections || (isAcceptingConnection && isHovered) || isAddingNewConnection;
...
function Pin({ isActive, ...props }) {
  return (
    <svg viewBox="0 0 24 24" {...props}>
      <circle cx={12} cy={12} r={9} strokeWidth={3} fill="transparent" className="stroke-current" />
...
export function NodeOutput({ outputID }) {
  ...
  const [isAcceptingConnection] = useAtom(getPinIsAcceptingConnections(outputID));
  const [isHovered, setIsHovered] = React.useState(false);
      ...
      >
        ...
        >
          <svg
            ...
            className={`h-3 w-3 ${isActive ? "text-blue-500" : "text-gray-500"}`}
          >
            <circle
              cx={12}
              cy={12}
