...
export function useQuery<Data = any, Variables = object>(
  ...
  const [state, setState] = useState(() => {
    currentInit = true;
    try {
      return [source, computeNextState(initialState, getSnapshot(source, suspense)), deps] as const;
    } finally {
      currentInit = false;
    }
  });
  ...
  if (source !== state[0] && hasDepsChanged(state[2], deps)) {
    setState([
      source,
      (currentResult = computeNextState(state[1], getSnapshot(source, suspense))),
      deps
    ]);
  }
