...
it("triggers correctly when the operations has an error", async () => {
  ...
  expect(onError).toBeCalledWith(new CombinedError({ networkError: error }), queryOperation);
});
...
it("triggers correctly multiple times the operations has an error", async () => {
  ...
  expect(onError).toBeCalledWith(new CombinedError({ networkError: error }), firstQuery);
  expect(onError).toBeCalledWith(new CombinedError({ networkError: error }), thirdQuery);
});
