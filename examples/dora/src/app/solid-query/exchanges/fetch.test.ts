...
describe("on error", () => {
  ...
  it("returns error data", async () => {
    const data = await pipe(fromValue(queryOperation), fetchExchange(exchangeArgs), toPromise);
...
describe("on teardown", () => {
  ...
  it("does not call the query", () => {
    pipe(
      fromValue(makeOperation("teardown", queryOperation, queryOperation.context)),
      fetchExchange(exchangeArgs),
      subscribe(fail)
    );
