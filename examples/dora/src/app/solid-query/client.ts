...
export const Client: new (opts: ClientOptions) => Client = function Client(
  ...
  const makeResultSource = (operation: Operation) => {
    let result$ = pipe(
      ...
      filter((res: OperationResult) => {
        if (res.operation.kind === "mutation") {
          ...
          return res.operation.kind === operation.kind && res.operation.key === operation.key;
        }
      })
    );
    ...
    const source = pipe(
      ...
      onEnd(() => {
        ...
        dispatchOperation(makeOperation("teardown", operation, operation.context));
      }),
      share
    );
  ...
  const client: Client = Object.assign(instance, {
        ...
        return pipe(
          ...
          onStart(() => {
            ...
            if (prevReplay != null && prevReplay === replays.get(operation.key)) {
              observer.next(isNetworkOperation ? { ...prevReplay, stale: true } : prevReplay);
            } else if (!isNetworkOperation) {
              dispatchOperation(operation);
            }
          }),
    ...
    query(query, variables, context) {
      ...
      return withPromise(client.executeQuery(createRequest(query, variables), context));
    },
    ...
    mutation(query, variables, context) {
      return withPromise(client.executeMutation(createRequest(query, variables), context));
    }
  } as Client);
