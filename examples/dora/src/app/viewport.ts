...
import { App } from "./app";
import { Bounds, Camera } from "~/state/types";
import Vec from "@tldraw/vec";
import { FIT_TO_SCREEN_PADDING } from "~/state/constants";
import { DIMENSIONS, PADDING } from "~/state/utils/iso";
...
export class Viewport {
  ...
  @observable viewBoxPosition = [0, 0];
  @observable viewBoxSize = { width: 0, height: 0 };
  @observable viewBoxOffset = [0, 0];
  ...
  @observable cameraPosition: number[] = [0, 0];
  @observable cameraZoom: number = 1;

  get camera(): Camera {
    return {
      point: this.cameraPosition,
  ...
  @action updateCamera(update: Partial<Camera>) {
    this.cameraPosition = update.point || this.camera.point;
    this.cameraZoom = update.zoom || this.camera.zoom;
    return this;
  }

  panCamera(delta: number[]) {
    this.cameraPosition = Vec.sub(this.cameraPosition, Vec.div(delta, this.cameraZoom));
    return this;
  }
